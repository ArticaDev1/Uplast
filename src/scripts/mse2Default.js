...
var mSearch2 = {
	...
	options: {
		...
		selected_tpl: `
			<a href="#" data-id="_id_" class="catalog-label catalog-label_small"> <span>_title_</span>
				<svg class="icon">
					<use xlink:href="/assets/components/project/uplast-main/build/img/icons/icons.svg#close-button"></use>
				</svg>
			</a>`,
	...
	numbers: {},
	selections: {},
	elements: ['filters', 'results', 'pagination', 'total', 'total_text', 'sort', 'selected', 'limit', 'tpl'],
	initialize: function (selector) {
		...
		if (this.selected) {
			...
			let $selectors = this.filters.find(selectors.join(', '));
			...
			$(document).on('click', this.options.selected + ' .catalog-label', function () {
				...
				$(document).on('click', this.options.pagination_link, function () {
					if (!$(this).hasClass(mSearch2.options.active_class)) {
						...
						mSearch2.load(params, function () {
							document.dispatchEvent(new CustomEvent("after_change_resaults_page"));
						});
					}
	...
	handleSelected: function (input) {
		...
		this.selected.find('.catalog-label').remove();
	...
	setTotal: function (total) {
		...
		let test = count % 100;
		if (test >= 5 && test <= 20) {
			this.total_text.text(declention[2]);
		} else {
			test = count % 10;
			if (test == 1) {
				this.total_text.text(declention[0]);
			} else if (test >= 2 && test <= 4) {
				this.total_text.text(declention[1]);
			} else {
				this.total_text.text(declention[2]);
			}
		}
	...
	beforeLoad: function () {
		document.dispatchEvent(new CustomEvent("before_load_filter_resaults"));
	...
	afterLoad: function () {
		document.dispatchEvent(new CustomEvent("after_load_filter_resaults"));
		...
		$(selector).each(function () {
			...
			form.find('input[name="' + config['queryVar'] + '"]').autocomplete({
					source: function (request, callback) {
						...
						//noinspection JSUnresolvedFunction
						console.log(mse2Config['actionUrl'], data)
...
function test_catalog_head(count) {
	let $parent_wrapper = $('.catalog-controls'),
		$labels_wrapper = $parent_wrapper.find('.catalog-controls__labels'),
		$items_wrapper = $parent_wrapper.find('.catalog-controls__items');

	if ($items_wrapper.length) return;

	if (count) {
		$parent_wrapper.removeClass('d-none d-lg-none');
	} else if ($labels_wrapper.children().length) {
		$parent_wrapper.addClass('d-lg-none');
	} else {
		$parent_wrapper.addClass('d-none');
	}
}