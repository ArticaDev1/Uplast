(function (window, document, $, miniShop2Config) {
  ...
  miniShop2.send = function (data, callbacks, userCallbacks) {
    ...
    let action;
    for(let key in data) {
      if (data[key].name == miniShop2.actionName) {
        action = data[key].value;
        break;
      } else if (key == miniShop2.actionName) {
        action = data[key];
        break;
      }
    }

    let $form = miniShop2.sendData.$form ? miniShop2.sendData.$form[0] : undefined;
    miniShop2.sendData.$form = undefined;
    ...
    //custom events
    document.dispatchEvent(new CustomEvent("miniShop2_callback", {
      detail: {
        action: action,
        $form: $form,
        type: 'before'
  ...
  miniShop2.Cart = {
    ...
    setup: function () {
      ...
      miniShop2.Cart.miniCartNotEmptyClass = 'header__cart-button_full';
      miniShop2.Cart.countInput = 'input[name=count]';
      miniShop2.Cart.totalWeight = '.ms2_total_weight';
      miniShop2.Cart.totalCount = '.ms2_total_count';
      miniShop2.Cart.totalPosition = '.ms2_total_position';
      miniShop2.Cart.totalText = '.ms2_total_text';
    ...
    initialize: function () {
      ...
      miniShop2.$doc.on('change input', miniShop2.Cart.cart + ' ' + miniShop2.Cart.countInput, function () {
        if (!!$(this).val()) {
    ...
    status: function (status) {
      console.log(status)
      if (status['total_count_position'] < 1) {
        location.reload();
      } else {
        var $miniCarts = $(miniShop2.Cart.miniCart).add(miniShop2.Cart.miniCartClass);
        if (status['total_count_position'] > 0 && $miniCarts.length > 0) {
          $miniCarts.each(function (index, cart) {
            if (!$(cart).hasClass(miniShop2.Cart.miniCartNotEmptyClass)) {
              ...
              //set href
              $(cart).attr('href', $(cart).attr('data-url'));
              $(cart).removeAttr('data-url');
              //destroy tippy
              if($(cart)[0]._tippy) {
                $(cart).removeAttr('data-tippy-content');
                $(cart)[0]._tippy.destroy();
              }
            }
          });
        }
        $(miniShop2.Cart.totalWeight).text(miniShop2.Utils.formatWeight(status['total_weight']));
        $(miniShop2.Cart.totalPosition).text(status['total_count_position']);
        ...
        //set text
        this.set_total_text(status['total_count_position']);
      }
    },
    clean: function () {
      var callbacks = miniShop2.Cart.callbacks;
    ...
    set_total_text: function (count) {
      let $total_text = $(miniShop2.Cart.totalText);

      if ($total_text.length) {
        let declention = $total_text.data('declension').split('|');
        //set text
        let test = count % 100;
        if (test >= 5 && test <= 20) {
          $total_text.text(declention[2]);
        } else {
          test = count % 10;
          if (test == 1) {
            $total_text.text(declention[0]);
          } else if (test >= 2 && test <= 4) {
            $total_text.text(declention[1]);
          } else {
            $total_text.text(declention[2]);
          }
        }
  ...
  miniShop2.Order = {
    ...
    setup: function () {
      ...
      miniShop2.Order.inputParent = '.input';
    ...
    add: function (key, value) {
      ...
      callbacks.add.response.success = function (response) {
        (function (key, value, old_value) {
          ...
          /* $field.val(response.data[key]).closest(miniShop2.Order.inputParent).removeClass('error'); */
        })(key, value, old_value);
      };
